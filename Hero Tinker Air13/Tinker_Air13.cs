       ...
       public static Vector3 FindVector(Vector3 first, double ret, float distance)
        ...
        static void Information(EventArgs args)
        {
            ...
            if (target != null && target.IsValid && !target.IsIllusion && target.IsAlive && target.IsVisible)
            {
                ...
                Drawing.DrawText(EZkill(target) ? procastdamage.ToString()+" ez" : procastdamage.ToString(), start2, new Vector2(21, 21), EZkill(target) ? Color.Lime : Color.Red, FontFlags.AntiAlias | FontFlags.Additive | FontFlags.DropShadow);
